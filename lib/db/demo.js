...
client.query("INSERT INTO users ( name, password, role) VALUES ('test1', '" + hasedpassword + "', 'admin')", (err, result) => {
  if (err) {
    console.log("query error", err.stack);
  }
});
...
client.query("INSERT INTO users ( name, password, role) VALUES ('test2', '" + hasedpassword + "', 'user')", (err, result) => {
  if (err) {
    console.log("query error", err.stack);
  }
  client.end()
